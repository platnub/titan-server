...
6. ```
   awk '{gsub(/disable_user_create_org: false/, "disable_user_create_org: true")}1' /opt/docker/pangolin-core/appdata/config/config.yml > tmp.yaml && mv tmp.yaml /opt/docker/pangolin-core/appdata/config/config.yml
   ```
